...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{route('service.create') . '?language=' . request()->input('language')}}" class="btn btn-primary shadow-sm btn-sm">
                <i class="fas fa-plus fa-sm me-1"></i> Novo Serviço
            </a>
        </div>
    </div>
    ...
    <div class="card shadow mb-4 border-0">
        ...
        <div class="card-body">
            <form action="{{route('delete.service')}}" method="POST" id="delete-services-form">
                @csrf
                @method('DELETE')
                    ...
                    <table class="table table-hover align-middle border-0" id="dataTable" width="100%" cellspacing="0">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th class="border-0">{{clean( trans('niva-backend.box_icon') )}}</th>
                        ...
                        <tbody>
                            @if($services)
                                @foreach($services as $service)
                                    <tr>
                                        <td><input class="checkboxes form-check-input" form="delete-services-form" type="checkbox" name="checkbox_array[]" value="{{$service->id}}"></td>
                                        <td>
                                            <img loading="lazy" width="80" class="rounded shadow-sm" src="{{$service->photo ? asset('images/media/' . $service->photo->file) : asset('img/200x200.png')}}" alt="">
                                        </td>
                                        <td>
                                            <div class="text-primary fs-4">{!!$service->icon!!}</div>
                                        </td>
                                        <td class="fw-bold">{{$service->title}}</td>
                                        <td class="text-muted small" style="max-width: 300px;">{{Str::limit($service->description, 100)}}</td>
                                        <td class="text-end">
                                            <div class="d-flex justify-content-end gap-1">
                                                <a href="{{ route('service.edit', $service->id) . '?language=' . request()->input('language')}}" class="btn btn-sm btn-outline-primary border-0 shadow-none">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('service.destroy', $service->id) }}" method="POST" class="d-inline single-delete-form">
                                                    @csrf
                                                    @method('DELETE')
...
<script>
    $(document).ready(function() {
        ...
        $('#delete-services-form').on('submit', function(e) {
            if ($('.checkboxes:checked').length === 0) {
                e.preventDefault();
                showToasty('Selecione pelo menos um serviço para excluir.', 'error');
                return;
            }
            ...
            Swal.fire({
                title: 'Tem certeza?',
                text: "Os serviços selecionados serão excluídos permanentemente!",
                icon: 'warning',
