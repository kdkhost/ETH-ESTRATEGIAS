...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{clean( trans('niva-backend.create_page') )}}</h1>
        <a href="{{route('page.index') . '?language=' . request()->input('language')}}" class="btn btn-primary shadow-sm">
            <i class="fas fa-arrow-left fa-sm text-white-50 me-1"></i> {{clean( trans('niva-backend.back_pages') )}}
        </a>
    </div>
    ...
    <div class="card shadow mb-4 border-0">
        <div class="card-header py-3 bg-white border-0">
            <div class="row align-items-center">
                <div class="col">
                    <h6 class="m-0 font-weight-bold text-primary">{{clean( trans('niva-backend.create_page') )}}</h6>
                </div>
                <div class="col-auto">
                    @if (!empty($langs))
