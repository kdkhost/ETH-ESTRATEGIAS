...
<div class="container-fluid">
    ...
    <div class="card gourmet-card-light shadow-sm border-0">
        <div class="card-header py-3 bg-white border-0 d-flex align-items-center">
            <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-list me-2"></i>{{clean( trans('niva-backend.all_sliders') )}}</h6>
            @if (!empty($langs))
                            ...
                            @if($sliders)
                                @foreach($sliders as $slider)
                                    <tr class="border-bottom-0">
                                        <td class="px-3"><input class="checkboxes form-check-input" form="delete-sliders-form" type="checkbox" name="checkbox_array[]" value="{{$slider->id}}"></td>
                                        <td>
                                            <img loading="lazy" width="100" class="rounded-3 shadow-sm" src="{{$slider->photo ? asset('images/media/' . $slider->photo->file) : asset('img/200x200.png')}}" alt="">
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-end px-3">
                                            <div class="d-flex justify-content-end gap-2">
                                                <a href="{{ route('slider.edit', $slider->id) . '?language=' . request()->input('language')}}" class="btn btn-sm btn-outline-primary border-0 rounded-circle p-2" title="Editar">
